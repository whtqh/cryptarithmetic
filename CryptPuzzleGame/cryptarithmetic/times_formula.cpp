...
times_formula::times_formula(string str_up, string str_down, string *str_in, string str_ans)
{
	...
	int k = (int)str_down.size();		//Diiferent to Plus Formula.
	Carry_Max = k - 1;
	NK = k;
	Track_Depth = 0;
...
times_formula::~times_formula()
{
	delete []result;
	delete []constant;
}
void times_formula::times_track_recurison()
{
	int temp_id = -1;
	...
	for (int i = 0; i < 10; i++)		//遍历所有可能进行搜索
	{
		if (result[temp_id].species_check[i] == 1)
		{
			...
			times_track_recurison();	//递归函数，有可能因为搜索失败返回并且没有能够初始化。
		}
		for (int j = 0; j < 10; j++)
		{
			result[temp_id].species_check[j] = RestoreCheck[j];
...
bool times_formula::find_goal_symbol()
{
	this->update_species();
	//缩小当前取值范围
...
bool times_formula::contradiction()
{
...
bool times_formula::CheckStatus()
{
	for (int i = 0; i < symbol_num; i++)
	{
		if (result[i].Known == false)
